...
**SRNetworkManager** is a Swift Package with one library target (`Sources/`) and one test target (`Tests/`). There is also an `Example/` Xcode project that demonstrates usage but is not part of the package.
...
- **SRNetworkManager/** — entry-point file (documentation only, no code)
